...
function WithDrawMoney() {
  ...
  const handleSubmit = async (e) => {
    ...
    await axios
      .put(`${server}/shop/update-payment-methods`, { withdrawMethod }, { withCredentials: true })
      .then((res) => {
  ...
  const error = () => {
    toast.error('You not hava enough balance to withdraw!');
  };

  const withdrawHandler = async () => {
    if (withdrawAmount < 50 || withdrawAmount > seller?.availableBalance) {
      toast.error('Your can withdraw this amount!');
    } else {
  ...
  return (
    <div className="w-full h-[90vh] p-8 ">
      <div className="w-full text-[20px] pb-4 bg-white h-full rounded flex items-center justify-center flex-col">
        <h5>Availble Balance: ${availableBalance}</h5>
        <div
      ...
      {open && (
        <div className="w-full h-screen fixed z-[9999] top-0 left-0 flex items-center justify-center bg-[#0000004e]">
          <div
          ...
          >
            ...
            {paymentMethod ? (
                ...
                <form onSubmit={handleSubmit}>
                  <div>
                    ...
                    <input
                      ...
                      // sao chép toàn bộ nội dung của bankInfo sử dụng spread operator ({ ...bankInfo }) và chỉ cập nhật giá trị của trường bankName
              ...
              <>
                <h3 className="text-[20px] font-Poppins">Available Withdraw Methods:</h3>
