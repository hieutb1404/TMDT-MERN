...
function UserOrderDetails() {
  ...
  const refundHandler = async () => {
    await axios
      .put(`${server}/order/order-refund/${id}`, { status: 'Processing refund' })
      .then((res) => {
        toast.success(res.data.message);
        dispatch(getAllOrdersOfUser(user._id));
      })
    ...
    <div className={`py-4 min-h-screen ${styles.section}`}>
        ...
        <div className="w-full fixed top-0 left-0 h-screen bg-[#0005] z-50 flex items-center justify-center">
          <div className="w-[50%] h-min bg-[#fff] shadow rounded-md p-3">
            <div className="w-full flex justify-end p-3">
              <RxCross1 size={30} onClick={() => setOpen(false)} className="curosr-pointer" />
            </div>
            ...
            <div className="w-full ml-3">
              ...
              <textarea
                ...
                rows="5"
                placeholder="Expresion about it!"
                className="mt-2 w-[95%] border p-2 outline-none"
              ></textarea>
              <div
                className={`${styles.button} text-white text-[20px] ml-3`}
                // phai danh gia sao tren 1 thi moi co the review
      ...
      <div className="w-full 800px:flex items-center">
        ...
        <div className="w-full 800px:w-[40%]">
          ...
          <br />
          {/* neu order = delivered thi moi hien button refund */}
